...
@Component
public class QimenDeliveryConfirmImpl implements QimenDeliveryConfirmStrategy {

  private static final Logger LOGGER = LoggerFactory.getLogger(QimenDeliveryConfirmImpl.class);
  @Autowired
  ...
  public DeliveryorderConfirmResponse confirm(OmsDeliveryOrderConfirmRequest request) {
    try {
      ...
      if (!Assert.isEmpty(deliveryOrder.getOrderConfirmTime())) {
        ...
        bo.setOutTime(
            DateTimeUtil.parserLocalDateTime(deliveryOrder.getOperateTime()));
      } else {
      ...
      for (OmsDeliveryOrderPackage orderPackage : omsDeliveryOrderPackages) {
        ...
        outPackage.setDeliveryTime(LocalDateTime.now());
      ...
      if (!usePackageDelivery && !Assert.isEmpty(request.getOrderLines())) {
        ...
        for (OmsDeliveryOrderOrderLine orderLine : request.getOrderLines()) {
          ...
          detail.setOutQuantity(ConvertUtil.toInt(orderLine.getActualQty()));
          details.add(detail);
        }
