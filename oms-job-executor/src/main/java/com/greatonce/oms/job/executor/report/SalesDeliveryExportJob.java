...
public class SalesDeliveryExportJob implements Job {

  private Logger LOGGER = LoggerFactory.getLogger(SalesDeliveryExportJob.class);
  @Value("${service.web.url:http://127.0.0.1:8181/api}")
  private String webUrl;
  private static String URL_SUFFIX = "/report/auto/exportSalesDeliveryForDay";
  @Resource
  ...
  public void execute(JobExecutionContext jobExecutionContext) {
    ...
    if (Assert.isNull(user)) {
      LOGGER.error("定时导出销售和补单(非代发)数据任务出错。原因：loginName不存在或错误！loginName:{}", loginName);
      JobKey jobKey = jobExecutionContext.getJobDetail().getKey();
      try {
        globalScheduler.pauseJob(jobKey);
        LOGGER.error("已停止定时导出销售和补单(非代发)数据任务。");
      } catch (SchedulerException e) {
        LOGGER.error("停止定时导出销售和补单(非代发)数据任务出错。异常信息：{}", e);
      }
    }
    Map<String, String> map = new HashMap<>(4);
    String fileName = "销售和补单(非代发)数据 " + DateTimeUtil.format(LocalDate.now().minusDays(days));
    map.put("fileName", fileName);
    map.put("userId", String.valueOf(user.getUserId()));
    map.put("deliveryTimeBegin", DateTimeUtil.format(
        LocalDateTime.of(LocalDate.now().minusDays(days), LocalTime.MIN)));
    map.put("deliveryTimeEnd",
